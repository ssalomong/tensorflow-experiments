...
def generateRandomData():
	...
	d1 = np.random.multivariate_normal((22,22), cov, 20)
	d2 = np.random.multivariate_normal((33,33), cov, 20)
	d3 = np.random.multivariate_normal((33,22), cov, 20)
	dataraw = pd.DataFrame(np.concatenate((d1,d2,d3)), columns=['x','y'])
	dataraw.to_csv('tempdata.csv')



if __name__ == '__main__':
	## Reads data from a file
	# dataraw = pd.read_csv('kpoints.csv')
	generateRandomData()
	dataraw = pd.read_csv('tempdata.csv')

	## Generate random data from a few gaussian distributions
	cov = np.array([[21,8], [8,21]])
	d1 = np.random.multivariate_normal((22,22), cov, 20)
	d2 = np.random.multivariate_normal((33,33), cov, 20)
	d3 = np.random.multivariate_normal((33,22), cov, 20)
